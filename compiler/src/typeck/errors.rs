...
pub fn field_not_found(db: &Db, ty: Ty, span: Span, field: Word) -> Diagnostic {
    Diagnostic::error()
        .with_message(format!("no field `{}` in type `{}`", field, ty.display(db)))
...
pub fn variant_not_found(db: &Db, ty: Ty, span: Span, variant: Word) -> Diagnostic {
    Diagnostic::error()
        .with_message(format!("no variant `{}` in type `{}`", variant, ty.display(db)))
        .with_label(
...
pub fn ty_arg_mismatch(expected: usize, found: usize, span: Span) -> Diagnostic {
    Diagnostic::error()
        .with_message(format!("expected {expected} type argument(s), but {found} were supplied"))
        .with_label(
            Label::primary(span)
...
pub fn invalid_un_op(db: &Db, op: UnOp, ty: Ty, span: Span) -> Diagnostic {
    Diagnostic::error()
        .with_message(format!("cannot use `{}` on type `{}`", op, ty.display(db)))
        .with_label(Label::primary(span).with_message(format!("invalid use of `{op}`")))
}
...
pub fn name_defined_twice(kind: &str, name: Word, prev_span: Span) -> Diagnostic {
    Diagnostic::error()
        .with_message(format!("the name `{name}` is already used as a {kind} name"))
...
pub fn invalid_attr_placement(attr: &ast::Attr, applies_to: &str) -> Diagnostic {
    Diagnostic::error()
        .with_message(format!("attribute `{}` should be applied to {}", attr.id, applies_to))
