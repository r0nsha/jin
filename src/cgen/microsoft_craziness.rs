...
unsafe fn visit_files_w(
    ...
    if handle != INVALID_HANDLE_VALUE {
        loop {
            ...
            {
                let short_name = U16CStr::from_ptr(
                    find_data.cFileName.as_ptr(),
                    wcslen(find_data.cFileName.as_ptr()),
                )
                .unwrap();
...
impl SafeHKey {
    fn new() -> Self {
        Self { inner: ptr::null_mut::<HKEY__>() }
    }
}
...
unsafe fn find_windows_kit_root(result: &mut FindResult) {
    ...
    if let Some(windows10_root) =
        read_from_the_registry(main_key.inner, u16cstr!("KitsRoot10").as_ptr())
    {
    ...
    else if let Some(windows8_root) =
        read_from_the_registry(main_key.inner, u16cstr!("KitsRoot81").as_ptr())
    {
...
unsafe fn find_visual_studio_2017_by_fighting_through_microsoft_craziness(
